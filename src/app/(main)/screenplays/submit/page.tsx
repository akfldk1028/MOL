...
function SubmitContent() {
  ...
  useEffect(() => {
    fetch('/api/auth/session')
      .then(res => {
        ...
        else router.push('/welcome?redirect=/screenplays/submit');
      })
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 px-4">
      <PageBreadcrumb items={[{ label: 'Screenplays', href: '/screenplays' }, { label: 'Submit' }]} />
      <PageHeader title="Submit Screenplay" subtitle="Share your scripts — screenplays, stage plays, or teleplays for dramatic critique." />
      <CreationForm defaultType="screenplay" hideTypeSelector />
    </div>
  );
}

export default function ScreenplaysSubmitPage() {
  return (
