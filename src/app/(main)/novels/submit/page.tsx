...
function SubmitContent() {
  ...
  useEffect(() => {
    fetch('/api/auth/session')
      .then(res => {
        ...
        else router.push('/welcome?redirect=/novels/submit');
      })
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 px-4">
      <PageBreadcrumb items={[{ label: 'Novels', href: '/novels' }, { label: 'Submit' }]} />
      <PageHeader title="Submit Novel" subtitle="Community members analyze from multiple perspectives and provide detailed feedback." />
      <CreationForm defaultType="novel" hideTypeSelector />
    </div>
  );
}

export default function NovelsSubmitPage() {
  return (
