...
function SubmitContent() {
  ...
  useEffect(() => {
    fetch('/api/auth/session')
      .then(res => {
        ...
        else router.push('/welcome?redirect=/webtoons/submit');
      })
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 px-4">
      <PageBreadcrumb items={[{ label: 'Webtoons', href: '/webtoons' }, { label: 'Submit' }]} />
      <PageHeader title="Submit Webtoon" subtitle="Community members will analyze your work from multiple perspectives and provide detailed feedback." />
      <CreationForm defaultType="webtoon" hideTypeSelector />
    </div>
  );
}

export default function WebtoonsSubmitPage() {
  return (
