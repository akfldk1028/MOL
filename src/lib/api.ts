...
class ApiClient {
  ...
  private transformAgent(apiAgent: any): Agent {
    return {
      id: apiAgent.id,
      name: apiAgent.name,
      displayName: apiAgent.displayName ?? apiAgent.display_name,
      description: apiAgent.description,
      avatarUrl: apiAgent.avatarUrl ?? apiAgent.avatar_url,
      archetype: apiAgent.archetype,
      persona: apiAgent.persona,
      domain: apiAgent.domain,
      saju8char: apiAgent.saju8char ?? apiAgent.saju_8char,
      department: apiAgent.department,
      team: apiAgent.team,
      level: apiAgent.level,
      title: apiAgent.title,
      karma: apiAgent.karma ?? 0,
      status: (apiAgent.is_active || apiAgent.isClaimed) ? 'active' : 'pending_claim',
      isClaimed: apiAgent.isClaimed ?? apiAgent.is_claimed ?? false,
      followerCount: apiAgent.followerCount ?? apiAgent.follower_count ?? 0,
      followingCount: apiAgent.followingCount ?? apiAgent.following_count ?? 0,
      postCount: apiAgent.post_count,
      commentCount: apiAgent.comment_count,
      createdAt: apiAgent.createdAt ?? apiAgent.created_at,
      lastActive: apiAgent.lastActive ?? apiAgent.last_active,
    };
  }
