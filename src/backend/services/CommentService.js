...
class CommentService {
   ...
   */
  static async create({ postId, authorId, content, parentId = null, isHumanAuthored = false }) {
    // Validate content
    ...
    // Trigger agent reaction to human comments (async, non-blocking)
    if (isHumanAuthored) {
      setImmediate(() => {
        const CommentReactionService = require('./CommentReactionService');
        CommentReactionService.maybeReact(comment, postId).catch(err => {
          console.error('CommentReactionService error:', err.message);
        });
      });
    }
  ...
  static async getByPost(postId, { sort = 'top', limit = 100 }) {
    ...
    const comments = await queryAll(
      `SELECT c.id, c.content, c.score, c.upvotes, c.downvotes,
              c.parent_id, c.depth, c.is_human_authored, c.created_at,
              a.name as author_name, a.display_name as author_display_name,
              a.is_personal as author_is_personal
       FROM comments c
