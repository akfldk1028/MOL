...
class ContextComposer {
  ...
  /**
   * B1: 진부한 클리셰 블랙리스트 — 라이브 ep1 직접 정독에서 발견된 패턴 차단.
   * 천마신공/철의 연금술사/나무와 쥐의 저주/별을 삼킨 자 ep1 모두 동일 클리셰 사용.
   * 이 룰은 author_intent (L2) 우선순위 80으로 주입.
   */
  addClicheBlacklist() {
    const text = [
      '## ⛔ 진부한 클리셰 금지 (예심 탈락 직행)',
      '',
      '다음 패턴은 첫 화/에피소드에 사용 금지. 사용 시 차별화된 변주 필수:',
      '- 갑자기 변한 몸 (철의 팔, 짐승화 등)',
      '- 잃어버린 후계자 / 무자각 왕족',
      '- 의문의 노인이 진실을 알려주는 폭로',
      '- 검은 나무 / 검은 석상 / 검은 불꽃 같은 막연한 어둠 묘사',
      '- 트럭에 치여 회귀 / 환생 / 빙의',
      '- "그 순간 모든 것이 달라졌다" 같은 추상 전환',
      '- "이건... 도대체 무엇인가?" 같은 추상 의문',
      '',
      '## ✅ 구체적 행동 / 구체적 감각으로 보여주기',
      '- 추상 감정 명명 금지 ("그는 혼란스러웠다") → 행동/대사로 ("그는 손바닥을 펴고 또 접었다")',
      '- "갑자기"·"문득"·"마침내"·"다행히" 같은 연결어 1회/3000자 제한',
      '- 캐릭터 내면을 작가가 요약하지 말고, 캐릭터가 직접 말하거나 행동하게 하기',
    ].join('\n');
    this._add('L2', 'genre/cliche_blacklist', 'Cliché blacklist — fail predicates', text);
  }
