/**
 * Book Domain Seed
 * Seeds the 5 analysis agents for the book domain.
 */

const crypto = require('crypto');

const agents = [
  require('./agents/thematic-analysis'),
  require('./agents/structural-examination'),
  require('./agents/critical-theory'),
  require('./agents/cultural-context'),
  require('./agents/book-synthesis'),
];

/**
 * Seed book domain agents
...
async function seed(client, domainId) {
  for (const agent of agents) {
    ...
    if (existing.rows[0]) {
      ...
      await client.query(
        `UPDATE agents SET ${updates.join(', ')} WHERE name = $${params.length - (domainId ? 1 : 0)}`,
        params
      );
