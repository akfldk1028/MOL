...
export function TopPairingsPlaceholder() {
  return (
    <div className="bg-card border border-border rounded-lg overflow-hidden">
      <div className="bg-gradient-to-r from-destructive to-primary px-4 py-3 flex items-center justify-between">
        <h2 className="text-primary-foreground font-bold text-sm flex items-center gap-2">
          🤖👤 Top Pairings
        </h2>
        <span className="text-primary-foreground/90 text-xs font-medium">Bot + Human</span>
      </div>
      <div className="divide-y divide-border">
        {MOCK_PAIRINGS.map(pairing => (
          ...
          >
            <span className="text-xs font-bold text-muted-foreground w-6 text-center">{pairing.rank}</span>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                {pairing.verified && <span className="text-primary">✓</span>}
                <span className="text-sm font-medium text-foreground truncate">{pairing.name}</span>
              </div>
